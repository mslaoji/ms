...
/**
 * 对象扩充
 * @param number {string} 身份证号码字符串
 * @returns {Object} 生日,性别,年龄,籍贯,省市县
 */
function getIdInfo(number){		
    var CARLENGTH = {
	"15":function(){
	    return "19"+ number.substr(6,6);
	},
	"18":function(){
	    return number.substr(6,8);
	}	
    }	
    var address = CITYDATA[number.substring(0,6)];
    var myDate = new Date();
    var month = myDate.getMonth() + 1;
    var day = myDate.getDate();
    age = myDate.getFullYear() - number.substring(6, 10) - 1;
    if (number.substring(10, 12) < month || number.substring(10, 12) == month && number.substring(12, 14) <= day) {
      age++;
    }
    return {
      birthday:CARLENGTH[number.length]().replace(/(.{4})(.{2})/,"$1-$2-"),
      sex:(number.substr(16, 1) | 0) % 2 == 1 ? '男' : '女',
      age:age,
      address:address,
      region:address.match(/.+?(省|市|自治区|自治州|县|区)/g)
    }
}
