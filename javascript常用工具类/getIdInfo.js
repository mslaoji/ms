...
function getIdInfo(number){
	  var birthday = '',sex = '',age = '',address='';
    if(number && number.length == 15){
      birthday = "19"+ number.substr(6,6);
    }else if(number && number.length == 18){
      birthday = number.substr(6,8);
    }

    if ((number.substr(16, 1) | 0) % 2 == 1) {
      sex = '男';
    } else {
      sex = '女';
    }
    ...
    address = CITYDATA[number.substring(0,6)];
    return {
